...
def cnn_model():
    ...
    model.add(Convolution2D(32, 3, 3, input_shape=(96, 96, 1))) # produces 32 images of shape 96x96 each
...
def fit_model():
    ...
    model = cnn_model()
    if PRETRAIN:
        model.load_weights('landmark_cnn_model_weights_300.hdf5')
    ...
    model.save_weights('checkpoint_best_with_landmarks_300.hdf5', overwrite=True)
...
def write_to_csv(y_pred):
    ...
    with open('test_set_for_landmarks.csv', 'r+') as f:
        with open('checkpoint_best_with_landmarks_300.csv', 'w') as outf:
...
def predict_model():
    ...
    model = cnn_model()
    model.load_weights('checkpoint_best_with_landmarks_300.hdf5')
